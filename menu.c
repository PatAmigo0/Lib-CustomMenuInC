...
int menus_amount = 0, depth_forbeind = 0;
menu** menus_array = NULL;
		...
		if (menus_array[i] == menu_to_clear)
			{
				...
				if (menus_amount <= 0)
				...
				else
					{
						...
						menus_array[0]->running = 1; // so we always have a backup menu and it wont break (i hope)
					}
				break;
			}
}
			...
			if (used_menu->need_redraw)
				{
					...
					/*
					    visible_size.X = (SHORT)current_width;
					    visible_size.Y = (SHORT)current_height;
					    SetConsoleScreenBufferSize(hBackBuffer, visible_size);
					*/
							...
							if (used_menu->header_policy)
								{
									...
									int spaces = (menu_size.X - 6) / 2;  // Center "MENU"
