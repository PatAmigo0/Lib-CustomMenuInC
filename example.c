...
void select_sort_type_arrays(int* array, int n)
{
	...
	// Helper function for common post-sort operations

	void common_after_sort(int* arr, int num, double elapsed_time)
	{
	...
	// Wrapper functions using the common helper
	void quick_sort_wrapper()
	{
		_copy((void*)array_copy, (void*)array, size);
		double ntick = tick();
		quick_sort_nongrowth(array_copy, 0, n-1);
		common_after_sort(array_copy, n, tick() - ntick);
	}

	void merge_sort_wrapper()
	{
		_copy((void*)array_copy, (void*)array, size);
		double ntick = tick();
		merge_sort_nongrowth(array_copy, 0, n-1);
		common_after_sort(array_copy, n, tick() - ntick);
	}

	void selection_sort_wrapper()
	{
		_copy((void*)array_copy, (void*)array, size);
		double ntick = tick();
		selection_sort_nondecreasing(array_copy, n);
		common_after_sort(array_copy, n, tick() - ntick);
	}

	void insertion_sort_wrapper()
	{
		_copy((void*)array_copy, (void*)array, size);
		double ntick = tick();
		insertion_sort_nondecreasing(array_copy, n);
		common_after_sort(array_copy, n, tick() - ntick);
	}

	void bubble_sort_wrapper()
	{
		_copy((void*)array_copy, (void*)array, size);
		double ntick = tick();
		bubble_sort_nonfixed_nongrowth(array_copy, n);
		common_after_sort(array_copy, n, tick() - ntick);
	}
